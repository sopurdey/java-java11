...
public class Lambda_01_Test {
    ...
    public void test_filter_by_age() throws Exception {
        ...
        PersonPredicate predicateAdult = p -> p.getAge() >= 18;
        
        List<Person> result = filter(personList, predicateAdult);
    ...
    public void test_filter_by_password() throws Exception {
        ...
        PersonPredicate predicatePassword = p -> p.getAge() > 49 && DigestUtils.sha512Hex(p.getPassword()).equals(passwordSha512Hex);
		
		List<Person> result = filter(personList, predicatePassword);
